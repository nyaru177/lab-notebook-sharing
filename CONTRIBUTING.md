...
3. 在分类目录下新建自己的姓名文件夹，再按命名规则保存 Notebook。
...
主题只需要聚焦一个明确问题。目录中的“可分享主题”是示例，不是限制；如果主题跨多个方向，放入最主要的目录，并在开头写出相关分类和关键词。个人文件夹使用自己的姓名，建议用姓名拼音或英文小写以避免路径问题；不要把个人 Notebook 直接放在模块根目录，也不要为每个 Notebook 重复创建一个文件夹。
